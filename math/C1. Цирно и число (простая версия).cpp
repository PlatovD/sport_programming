...
ll build_one_rank_less_biggest(vector<ll> &digits_of_original, set<ll> &allowed) {
...
ll build_one_rank_more_lowest(vector<ll> &digits_of_original, set<ll> &allowed) {
    ll power = 1;
    ll res = 0;
...
void solve() {
    ...
    ll a_a = abs(a - build_one_rank_less_biggest(digits_of_original, allowed));
    ll b_a = abs(a - build_one_rank_more_lowest(digits_of_original, allowed));
    ll c_a = abs(build_same_rank_best(digits_of_original, allowed, a));


    cout << min(min(a_a, b_a), c_a) << '\n';
}
